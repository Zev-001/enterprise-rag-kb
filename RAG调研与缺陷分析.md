...
18. **D20 召回前元数据过滤（GEOFlow 知识治理对标）**：✅ 新增 `meta_filter.py`（2026-08-31）。

    - **抓什么**：D18/D19 的质检都发生在「答案生成之后」；GEOFlow 的另一个启发是
      **治理前移**——明年才生效的薪级表、已作废的报销标准、未定稿的草稿，检索得再准
      也不该进今天的上下文。这类块在【进入检索之前】就被拦掉。
    - **硬过滤不靠 prompt**：块带 `effective_date`/`effective_until`/`review_status`
      元数据时，未生效/已过期/草稿/待审核/已作废在语义索引与 TF-IDF 召回前剔除，
      GraphRAG 同样按可见块集合收口；写了 `review_status` 但值不认识 → fail-closed 保守拦截。
    - **向后兼容**：不写治理字段的老库行为完全不变；边界含当日（当天生效/当天到期均可用）；
      日期多格式解析（ISO/斜杠/中文/紧凑8位）。
    - **治理可观测**：`/api/stats` 返回拦截统计（前端状态栏「🛡 已拦截 N 块」），
      每次问答响应带 `meta_filtered`（拦了哪些块、各什么原因）；全拦时空手而归，
      交给防幻觉闸门走「资料里没提到」。
    - **开关**：`RAG_META_FILTER=off` 退回不过滤；`RAG_TODAY` 可指定「今天」做时间旅行演示。
  - **自测**：`test_meta_filter.py` 43 项全过（离线零依赖）；
    P2 35/35、D18 41/41、D19 30/30、eval 100 分均无回归。

- **升级④ 内容指纹缓存失效（D21，✅ 已完成 2026-09-01）**
    - **对标 GEOFlow `input_fingerprint`**：D15 答案缓存原来只有「TTL 过期」一条失效路径，
      知识库改了制度文件，昨天的答案今天还在用——人工记得清缓存才不踩坑，这是纪律问题不是机制问题。
    - **机制**：`rag_core.kb_fingerprint`（知识库文件字节 md5 + mtime/size 缓存，不重复读盘）
      计算当前库指纹；`cache.py` 每条缓存记录写入时绑定指纹，读取时指纹不一致
      → **条目当场销毁并返回 None**（自动失效，无需手动 `cache_invalidate`）。
    - **兼容与安全**：不传指纹的旧式调用完全不受影响；没存指纹的老条目，
      在传入指纹的新调用下**宁失效不答旧**（怕旧答案污染新库）。
    - **可观测**：`/api/stats` 返回 `kb_fp`（当前库版本），问答响应带 `kb_fp`，
      `/api/cache` 统计含指纹分布。
    - **自测**：`test_cache_fingerprint.py` 15 项全过（含端到端：手工灌旧缓存 → 真实新指纹读 → 命中失效）；
      P2 35/35、D18 41/41、D19 30/30、D20 43/43、eval 100 分均无回归。
